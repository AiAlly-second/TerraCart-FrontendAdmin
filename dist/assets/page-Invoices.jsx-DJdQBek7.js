import{r as a,j as t}from"./vendor-react-DLMngTKH.js";import{h as Y,E as J}from"./vendor--JSWM6CN.js";import{a as L}from"./page-CartDetails.jsx-CxpabDuf.js";const X=(i=[])=>{const o=new Map;return(i||[]).forEach(m=>{(m?.items||[]).forEach(l=>{if(!l)return;const d=l.name||"Item",x=Number(l.quantity)||0,s=Number(l.price||0)/100,g=!!l.returned;o.has(d)||o.set(d,{name:d,unitPrice:s,quantity:0,returnedQuantity:0,returned:!1,amount:0});const r=o.get(d);g?(r.returnedQuantity+=x,r.returned=!0):(r.quantity+=x,r.amount+=s*x),r.unitPrice||(r.unitPrice=s)})}),Array.from(o.values())},Z=(i=[],o=[])=>{const m=o.reduce((s,g)=>{const r=Number(g.amount)||0;return s+r},0),l=Number(m.toFixed(2)),d=Number((l*.05).toFixed(2)),x=Number((l+d).toFixed(2));return{subtotal:l,gst:d,totalAmount:x,totalItems:o.reduce((s,g)=>s+(Number(g.quantity)||0),0)}},w=i=>{const o=Number(i);return Number.isNaN(o)?"0.00":o.toFixed(2)},ee=(i,o,m,l,d,x)=>{if(!i)return"";const s=(()=>{const p=new Date(i.createdAt||Date.now()).toISOString().slice(0,10).replace(/-/g,""),b=(i._id||"").toString().slice(-6).toUpperCase();return`INV-${p}-${b}`})(),g=d?.address||"—",r=l?.gstNumber||"—",D=x||"CASH",P=o.length>0?o.map(p=>{const b=p.quantity||0,y=p.unitPrice||0,N=p.amount||0;return`
              <tr>
