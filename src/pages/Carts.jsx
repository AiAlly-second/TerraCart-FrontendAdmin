...
const Carts = () => {
  ...
  const fetchCarts = async () => {
    try {
      ...
      // Filter admin users (cart managers)
      const adminUsers = allUsers.filter((user) => user.role === "admin");
      
      // Transform to cart format
      const cartsData = adminUsers.map((user) => ({
        id: user._id,
        name: user.cartName || user.name || "Unnamed Cafe",
        ...
        status: !user.isApproved ? "Pending Approval" : (user.isActive !== false ? "Active" : "Inactive"),
  ...
  const handleReject = async (cartId) => {
    if (!window.confirm("Are you sure you want to reject this cart admin? This action cannot be undone.")) {
      return;
    }
    try {
  ...
  const filteredCarts = carts.filter((cart) => {
    const matchesSearch =
      ...
      (cart.cartCode && cart.cartCode.toLowerCase().includes(searchTerm.toLowerCase())) ||
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  {cart.isApproved && (
                    <button
                      ...
                      } ${togglingId === cart.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                      title={cart.isActive ? 'Deactivate Cart' : 'Activate Cart'}
                    >
                      <span
      ...
      {!loading && (
        <div className="mt-6 bg-white rounded-lg shadow-md p-4">
          <p className="text-sm text-gray-600">
            Showing <span className="font-semibold">{filteredCarts.length}</span>{" "}
            of <span className="font-semibold">{carts.length}</span> carts
            ...
            {carts.filter(c => c.isApproved && !c.isActive).length > 0 && (
              <span className="ml-2 text-red-600">
                ({carts.filter(c => c.isApproved && !c.isActive).length} inactive)
              </span>
            )}
          </p>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="border-b border-[#e2c1ac] pb-6">
                <h3 className="text-lg font-semibold text-[#4a2e1f] mb-4">Basic Information</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-[#6b4423] mb-1">Manager Name</label>
                    <p className="text-[#4a2e1f] font-medium">{approvalData.name || "N/A"}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[#6b4423] mb-1">Email</label>
                    <p className="text-[#4a2e1f] font-medium">{approvalData.email || "N/A"}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[#6b4423] mb-1">Cart Name</label>
                    <p className="text-[#4a2e1f] font-medium">{approvalData.cartName || "N/A"}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[#6b4423] mb-1">Location</label>
                    <p className="text-[#4a2e1f] font-medium">{approvalData.location || "N/A"}</p>
                  </div>
                  {approvalData.phone && (
                    <div>
                      <label className="block text-sm font-medium text-[#6b4423] mb-1">Phone</label>
                      <p className="text-[#4a2e1f] font-medium">{approvalData.phone}</p>
                    </div>
                  )}
                  {approvalData.address && (
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-[#6b4423] mb-1">Address</label>
                      <p className="text-[#4a2e1f] font-medium">{approvalData.address}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Documents Section */}
              <div className="border-b border-[#e2c1ac] pb-6">
                <h3 className="text-lg font-semibold text-[#4a2e1f] mb-4">Owner Documents</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Aadhar Card */}
                      ...
                      <div className="space-y-2">
                        ...
                        {approvalData.shopActLicenseExpiry && (
                          <p className="text-xs text-[#6b4423]">
                            Expiry: {formatDate(approvalData.shopActLicenseExpiry)}
                          </p>
                        )}
                      </div>
                    ) : (
                    ...
                    {approvalData.fssaiLicense ? (
                      <div className="space-y-2">
                        ...
                        {approvalData.fssaiLicenseExpiry && (
                          <p className="text-xs text-[#6b4423]">
                            Expiry: {formatDate(approvalData.fssaiLicenseExpiry)}
                          </p>
                        )}
                      </div>
                    ) : (
              ...
              <div className="text-sm text-[#6b4423]">
                <p><span className="font-medium">Registration Date:</span> {formatDate(approvalData.createdAt)}</p>
              </div>
            </div>
            ...
            <div className="sticky bottom-0 bg-white border-t border-[#e2c1ac] p-6 flex gap-4 justify-end">
              ...
              >
                {approvingId === approvalModal ? "Approving..." : "Approve Cart Admin"}
              </button>
            </div>
          </div>
        </div>
