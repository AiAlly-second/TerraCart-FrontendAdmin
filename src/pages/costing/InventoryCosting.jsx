...
const InventoryCosting = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      handleCloseModal();
      fetchIngredients();
      alert(`Ingredient ${editingIngredient ? 'updated' : 'created'} successfully!`);
    } catch (error) {
      console.error('Failed to save ingredient:', error);
      alert(`Failed to ${editingIngredient ? 'update' : 'create'} ingredient: ${error.response?.data?.message || error.message}`);
    }
  };
  ...
  const handleAdjustSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Failed to record adjustment: ${error.response?.data?.message || error.message}`);
    }
  };
        ...
        <div className="bg-white rounded-lg shadow-md border border-[#e2c1ac] overflow-x-auto">
          <table className="min-w-full">
            <thead className="bg-[#f5e3d5]">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-[#4a2e1f] uppercase">Name</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-[#4a2e1f] uppercase">Unit</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-[#4a2e1f] uppercase">Cost Per Unit</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-[#4a2e1f] uppercase">Last Updated</th>
                  ...
                  <tr key={ingredient._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => setDeleteModal({ isOpen: true, id: ingredient._id })}
        ...
        <>
          ...
          <div className="bg-white rounded-lg shadow-md p-4 border border-[#e2c1ac]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              />
              <div>
                <label className="block text-sm font-medium text-[#6b4423] mb-1">Change Type</label>
                <select
            ...
            <table className="min-w-full">
              <thead className="bg-[#f5e3d5]">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#4a2e1f] uppercase">Change Type</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#4a2e1f] uppercase">Quantity</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#4a2e1f] uppercase">Cost</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#4a2e1f] uppercase">Remarks</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                    ...
                    <tr key={transaction._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm">
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          transaction.changeType === 'purchase' ? 'bg-green-100 text-green-800' :
                          transaction.changeType === 'wastage' ? 'bg-red-100 text-red-800' :
                          transaction.changeType === 'consumption' ? 'bg-blue-100 text-blue-800' :
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">
                        {transaction.changeQty > 0 ? '+' : ''}{transaction.changeQty} {transaction.ingredientId?.unit || ''}
                      </td>
                      <td className="px-6 py-4 text-sm font-semibold text-[#4a2e1f]">
                        {formatCurrency(transaction.cost)}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-600">{transaction.remarks || '—'}</td>
                    </tr>
                  ))
                )}
              </tbody>
          ...
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-lg w-full mx-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-[#6b4423] mb-1">Cost Per Unit *</label>
                  <input
                    type="number"
                    step="0.01"
                    required
              ...
              <div>
                <label className="block text-sm font-medium text-[#6b4423] mb-1">Vendor ID</label>
                <input
                  type="text"
            ...
            <form onSubmit={handleAdjustSubmit} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setAdjustFormData({ ...adjustFormData, ingredientId: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#d86d2a]"
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setAdjustFormData({ ...adjustFormData, changeType: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#d86d2a]"
                  >
                    {changeTypes.map(type => (
                      <option key={type.value} value={type.value}>{type.label}</option>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setAdjustFormData({ ...adjustFormData, changeQty: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#d86d2a]"
              ...
              <div>
                <label className="block text-sm font-medium text-[#6b4423] mb-1">Cost</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
                  value={adjustFormData.cost}
                  onChange={(e) => setAdjustFormData({ ...adjustFormData, cost: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#d86d2a]"
                  placeholder="Optional"
                />
              </div>
